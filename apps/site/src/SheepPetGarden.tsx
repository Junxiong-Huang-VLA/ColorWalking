...
export function SheepPetGarden() {
  ...
  const runAction = (updater: (state: PetState) => PetState, message: string) => {
    setPet((prev) => {
  ...
  const play = () =>
    runAction(
      (s) => gainXp({ ...s, hunger: clamp(s.hunger + 8), energy: clamp(s.energy - 14), cleanliness: clamp(s.cleanliness - 6) }, 20),
      "\u4f60\u966a\u5b83\u8dd1\u5708\u73a9\u7403\uff0c\u6bdb\u5377\u98de\u8d77\u6765\u4e86\u3002"
    );
  ...
  return (
    <section id="pet" className="section pet-card">
      <h2>{"\u5c0f\u7f8a\u5377\u517b\u6210\u8231"}</h2>
      <p className="pet-desc">{"\u6bcf\u5929\u7167\u987e\u4e00\u4e0b\u4f60\u7684\u5c0f\u7f8a\u5377\uff0c\u5b83\u4f1a\u968f\u7740\u4f60\u7684\u966a\u4f34\u4e00\u8d77\u957f\u5927\u3002"}</p>

      <div className="pet-layout">
        <div className="pet-avatar-box">
          <div className="pet-avatar" style={{ ["--accent" as string]: favoriteColor?.hex ?? "#ffd93d" }}>
            <svg viewBox="0 0 240 190" role="img" aria-label="sheep-roll">
              ...
              <circle cx="106" cy="102" r="4" fill="#1f2a44" />
              <circle cx="134" cy="102" r="4" fill="#1f2a44" />
              <path d="M106 124 Q120 136 134 124" fill="none" stroke="#1f2a44" strokeWidth="4" strokeLinecap="round" />
